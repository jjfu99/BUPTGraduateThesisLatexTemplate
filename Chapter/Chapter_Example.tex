\chapter{例子展示}

一些基本使用的例子

\section{一级标题}
\subsection{二级标题}
\subsubsection{三级标题}

\section{图表和表格}

目前只实现了最基本的应用，没有实现官方word模板中使用题注的方式介绍详细图表信息。引用图\ref{fig:bupt_logo}和表\ref{tab:bupt_table}信息。

参考文献\cite{ddpg}。

\begin{figure}[htbp]
  \centering
  %\vspace{1 cm} % 使用该指令调整上下间距
  \includegraphics[width=0.5\textwidth]{resources/logo.png}
  \bicaption{图标题}{Figure Caption} % 使用中英文标题
  %\caption{中文标题} % 只使用中文标题
  \label{fig:bupt_logo}
  %\vspace{1 cm} % 使用该指令调整上下间距
\end{figure}

\setltcaption{跨页长表格表题} % 使用中文标题
% \setltcaption[Cross-page Long Table Caption]{跨页长表格表题} % 使用中英文标题
\begin{longtable}{cccc}
    \ltcaption % 使用中文标题
    %\ltbicaption % 使用中英文标题
    \toprule
    表头 1 & 表头 2 & 表头 3 & 表头 4 \\
    \midrule
  \endfirsthead
    \ltcaptionc % 使用中文标题
    %\ltbicaptionc % 使用中英文标题
    \toprule
    表头 1 & 表头 2 & 表头 3 & 表头 4 \\
    \midrule
  \endhead
    \bottomrule
  \endfoot
  \label{tab:longtable}
  Row 1  & & & \\
  Row 2  & & & \\
  Row 3  & & & \\
  Row 4  & & & \\
  Row 5  & & & \\
  Row 6  & & & \\
  Row 7  & & & \\
  Row 8  & & & \\
  Row 9  & & & \\
  Row 10 & & & \\
\end{longtable}

\begin{table}[htbp]
  \centering
  %\vspace{1 cm} % 使用该指令调整上下间距
  \bicaption{短表表题}{Short Table Caption}
  \renewcommand\arraystretch{1.5}
  \begin{tabular}{>{\centering\arraybackslash}m{4cm} >{\centering\arraybackslash}m{4cm} >{\centering\arraybackslash}m{4cm}}
    \toprule
    符\quad 号 & 代表意义 & 单\quad 位 \\
    $A$      & 截面积  & $m^{2}$  \\
    \bottomrule
  \end{tabular}
  \label{tab:short_table}
  %\vspace{1 cm} % 使用该指令调整上下间距
\end{table}


\section{公式}

\begin{equation}
  a + b = c
\end{equation}


\section{算法}

目前算法的标题是中文。% 如果需要变成英文Algorithm，在`buptthesis.cls'文件当中注释掉`\renewcommand{\ALG@name}{算法}`这个代码。
算法模块默认导入`algorithm'和`algorithmic'库，如果使用`algorithm2e'会发生冲突。引用算法\ref{alg:sum}信息。

\begin{algorithm}[htbp]
    \caption{algorithm of SUM}
    \label{alg:sum}
    \renewcommand{\algorithmicrequire}{\textbf{Input:}}
    \renewcommand{\algorithmicensure}{\textbf{Output:}}
    \begin{algorithmic}[1]
        \REQUIRE $A$, $B$, $C$  %%input
        \ENSURE EEEEE    %%output
        
        \STATE  AAAAA
        \WHILE{$A=B$}
            \STATE BBBBB
        \ENDWHILE
        
        \FOR{each $i \in [1,10]$}
            \IF {$C = 0$}
                \STATE CCCCC
            \ELSE
                \STATE 使用中文
            \ENDIF
        \ENDFOR
        
        \RETURN EEEEE
    \end{algorithmic}
\end{algorithm}